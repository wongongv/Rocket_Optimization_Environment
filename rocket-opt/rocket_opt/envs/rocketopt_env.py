...
VIEWPORT_W = 900
VIEWPORT_H = 600
BITS = {'rocket': 0x0001, 'target_planet':0x0002, 'obstacles':0x0004}
...
#make random stars
		...
		fix = fixtureDef(shape = polygonShape(vertices = self.planet_poly),
											density = self.planet_density, 
											categoryBits = BITS['target_planet'],
											maskBits = BITS['rocket'] | BITS['obstacles'],
											restitution=0.0)
		...
		self.rocket = self.world.CreateDynamicBody(
			...
			fixtures = fixtureDef(
				...
				density=1.0,
				# friction=0.1,
				categoryBits=BITS['rocket'],
				maskBits=BITS['target_planet'] | BITS['obstacles'],  # collide only with planets and obstacles
				restitution=0.0))
